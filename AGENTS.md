...
14. Prefer restarting between tracks, but the owner authorizes restarting the managed service during separation, rendering, or upload when needed to deploy fixes, stop incorrect uploads, or recover a stuck worker; do not ask again solely because a stage is active. Be aware that an interrupted stage may lose processing time and need retrying. Inspect the current stage first, preserve completed records, verify restart/recovery, and never launch a second pipeline. SQLite transactions protect committed records, but this is not a guarantee of zero risk: inspect incomplete files, and reconcile an upload that may have reached YouTube before its local commit before retrying it.
